...
# Check manifest*.yml
for mf in "$SOURCE_DIR"/manifest*.yml; do
    [ -e "$mf" ] || continue
    
    # Improved name extraction (strip leading space and handle potential \r)
    # Prefer manifest.yml for name or if name is not set
    if [[ "$(basename "$mf")" == "manifest.yml" ]] || [ "$APP_NAME" == "$(basename "$SOURCE_DIR")" ]; then
        M_NAME=$(grep "name:" "$mf" | head -1 | sed 's/.*name: *//' | tr -d '\r' | xargs)
        [ ! -z "$M_NAME" ] && APP_NAME=$M_NAME
    fi
    
    BUILDPACK=$(grep "buildpack:" "$mf" | head -1)
    ...
    # Extract Java Version from JBP_CONFIG_OPEN_JDK_JRE
    JBP_CONFIG=$(grep "JBP_CONFIG_OPEN_JDK_JRE:" "$mf" | head -1 | tr -d '\r')
    if [ ! -z "$JBP_CONFIG" ]; then
        JV=$(echo "$JBP_CONFIG" | sed -n 's/.*version: *\([0-9]\+\).*/\1/p')
        if [ ! -z "$JV" ]; then
            JAVA_VER=$JV
            APP_TYPE="java-maven"
            echo "Found Java version $JAVA_VER in $(basename "$mf")"
        fi
    fi
done
